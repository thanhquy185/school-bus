...
const FirebaseService = {
  ...
  async uploadParentImage(file) {
    try {
      if (!file) throw new Error("No file provided");
      const imageName = `parents/${Date.now()}_${file.originalname}`;
      const firebaseFile = bucket.file(imageName);
      await new Promise((resolve, reject) => {
        const stream = firebaseFile.createWriteStream({
          metadata: {
  ...
  async uploadDriverImage(file) {
    try {
      if (!file) throw new Error("No file provided");
      const imageName = `drivers/${Date.now()}_${file.originalname}`;
      const firebaseFile = bucket.file(imageName);
      await new Promise((resolve, reject) => {
        const stream = firebaseFile.createWriteStream({
          metadata: {
