                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        {/* Pickup location */}
                        {activePickups && activePickups.map((activePickup, index) => {
                          if (busLocation) {
                            if (activePickup.pickup.lat === busLocation.lat && activePickup.pickup.lng === busLocation.lng) return null;
                          ...
                          if (activePickup.pickup.category === "SCHOOL") {
                            return (
                              <Marker
                                key={`school-${index}`}
                                position={activePickup.pickup ? new LatLng(activePickup.pickup.lat, activePickup.pickup.lng) : new LatLng(0, 0)}
                                icon={schoolIcon}
                              >
                                <Popup>
                                  <p>Trường học: {activePickup.pickup.name}</p>
                                  <p>Tọa độ: {activePickup.pickup.lat}, {activePickup.pickup.lng}</p>
                                </Popup>
                              </Marker>
                            )
                          }
                          ...
                          return (
                            ...
                            >
                              <Popup>
                                ...
                                <p>Tọa độ: {activePickup.pickup.lat}, {activePickup.pickup.lng}</p>
                                <p>Trạng thái: {
                                  activePickup.status === "CONFIRMED" ? "Đã xác nhận" :
                                    activePickup.status === "DRIVING" ? "Đang di chuyển" :
                                      activePickup.status === "CANCELED" ? "Đã hủy" : "Chờ xử lý"
                                }</p>
                              </Popup>
                            </Marker>
                          )
